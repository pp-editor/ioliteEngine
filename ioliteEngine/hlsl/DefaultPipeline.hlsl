...
float4 blinnPhong(GSPSInput input) {
	...
	float3 iA = Mate_ambient.xyz * ALight_color.xyz;
...
//	color = float4(depthValue, depthValue, depthValue, 1.f);
	...
	if (saturate(projCoord.x) == projCoord.x && (saturate(projCoord.y) == projCoord.y)) {
		float depthValue = DepthMap.Sample(SamplerDepth, projCoord.xy).r;
		float lightDepth = projCoord.z - bias;
		if (lightDepth < depthValue) {
			float intensity = saturate(dot(input.Normal, input.LightPos));
			if (intensity > 0.0f) {
				color = saturate(color + DLight_color * intensity);
			}
		}
	}
	return color;
...
//	output = Texture.Sample(Sampler, input.TexCoord) * input.Color * shadowMapping(input) * blinnPhong(input);
	output = shadowMapping(input);
	clip(output.a - 0.0001f); 
}

void psMainNoTexture(in GSPSInput input, out float4 output : SV_TARGET0) {
//	output =  input.Color * shadowMapping(input);// * blinnPhong(input);
	output = shadowMapping(input);
	clip(output.a - 0.0001f);
}

void psMainCube(in GSPSInput input, out float4 output : SV_TARGET0) {
//	output = CubeTex.Sample(Sampler, input.CubeCoord) * input.Color * shadowMapping(input) * blinnPhong(input);
	output = shadowMapping(input);
	clip(output.a - 0.0001f);
}
...
void psMainCubeMap(in GSPSInput input, out float4 output : SV_TARGET0) {
	...
	output = CubeTex.Sample(Sampler, R) * input.Color * blinnPhong(input);
}
...
void psMainDepth(in GSPSInput input, out float4 output : SV_TARGET0) {
	float depthValue = input.Position.z / input.Position.w;
	output = float4(depthValue, depthValue, depthValue, 1.0f);
}